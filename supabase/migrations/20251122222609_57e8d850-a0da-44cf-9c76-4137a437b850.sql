-- Fix credit limit validation to include pending transactions
-- Credit cards should reserve credit for pending expenses, not just completed ones
...
RETURNS TABLE(transaction_id uuid, new_balance numeric, success boolean, error_message text)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Cartões de crédito devem reservar crédito mesmo para transações pendentes
  IF v_account_type = 'credit' AND p_type = 'expense' THEN
    ...
    BEGIN
      -- Dívida atual (completed transactions)
      v_current_debt := ABS(LEAST(v_account_balance, 0));
      
      -- Somar despesas pendentes
      ...
      -- Total de dívida (completed + pending)
