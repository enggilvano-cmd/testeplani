-- Fix scope handling for fixed/recurring child transactions in atomic_delete_transaction and atomic_update_transaction
...
CREATE OR REPLACE FUNCTION public.atomic_update_transaction(p_user_id uuid, p_transaction_id uuid, p_updates jsonb, p_scope text DEFAULT 'current'::text)
RETURNS TABLE(updated_count integer, affected_accounts uuid[], success boolean, error_message text)
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $function$
