-- Fix fixed transactions logic to create:
-- Current month + remaining months of current year + 12 months of next year
...
DECLARE
  ...
  v_end_of_current_year DATE;
  v_end_of_next_year DATE;
BEGIN
  ...
  -- Extrair dia, ano e mês da data inicial
  v_day_of_month := EXTRACT(DAY FROM p_date);
  v_current_year := EXTRACT(YEAR FROM p_date);
  v_current_month := EXTRACT(MONTH FROM p_date);

  -- Calcular fim do ano corrente e fim do ano subsequente
  v_end_of_current_year := DATE (v_current_year || '-12-31');
  v_end_of_next_year := DATE ((v_current_year + 1) || '-12-31');
  ...
  -- Loop até o fim do ano subsequente
  LOOP
    ...
    -- Parar quando ultrapassar o fim do ano subsequente
    EXIT WHEN v_current_date > v_end_of_next_year;
