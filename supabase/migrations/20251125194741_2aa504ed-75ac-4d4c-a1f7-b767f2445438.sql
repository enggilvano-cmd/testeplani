-- Fix atomic_delete_transaction to properly handle completed transactions in all scope
-- The function was only checking if the main transaction's period is locked, 
-- but not checking child transactions that might be in locked periods
...
BEGIN
  ...
  -- ✅ FIX: Validar TODAS as transações que serão deletadas, não apenas a principal
