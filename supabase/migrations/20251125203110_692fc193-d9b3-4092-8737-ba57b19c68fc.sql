-- Corrigir atomic_update_transaction para sincronizar ambas transações em transferências
...
BEGIN
  ...
  BEGIN
    ...
    IF v_linked_transaction_id IS NOT NULL THEN
      -- Buscar o tipo da transação vinculada para aplicar o sinal correto
      SELECT type INTO v_linked_type
      FROM transactions
      WHERE id = v_linked_transaction_id AND user_id = p_user_id;

      -- Se o amount foi atualizado, calcular o valor correto para a transação vinculada
      IF p_updates ? 'amount' THEN
        v_linked_amount := (p_updates->>'amount')::NUMERIC;
        -- Aplicar sinal oposto (se uma é expense, a outra é income)
        IF v_linked_type = 'expense' THEN
          v_linked_amount := -ABS(v_linked_amount);
        ELSIF v_linked_type = 'income' THEN
          v_linked_amount := ABS(v_linked_amount);
        END IF;
      END IF;
...
COMMENT ON FUNCTION public.atomic_update_transaction IS 'Atualiza transação(ões) sincronizando transferências vinculadas e recalcula saldos atomicamente';