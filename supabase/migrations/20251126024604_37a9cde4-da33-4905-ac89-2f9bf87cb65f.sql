-- Fix atomic_delete_transaction para preservar transações concluídas ao deletar parent de série fixa
-- Quando deletamos um parent de transação fixa com scope='current', devemos deletar SOMENTE o parent
-- sem afetar as transações filhas (especialmente as concluídas)
...
BEGIN
  ...
  ELSIF (v_is_fixed = true OR v_is_recurring = true) AND v_parent_id IS NOT NULL THEN
    ...
    IF p_scope = 'current' THEN
      v_transaction_ids := ARRAY[p_transaction_id];
    ELSIF p_scope = 'current-and-remaining' THEN
      -- Para transações fixas, deletar esta e as próximas PENDENTES baseado na data
      SELECT ARRAY_AGG(id) INTO v_transaction_ids
      FROM transactions
      WHERE parent_transaction_id = v_parent_id
        ...
        AND user_id = p_user_id;
    ELSE -- 'all'
      -- Quando scope='all' em transações fixas, deletar parent + TODAS as filhas PENDENTES
      SELECT ARRAY_AGG(id) INTO v_transaction_ids
      FROM transactions
  ...
  ELSIF (v_is_fixed = true OR v_is_recurring = true) THEN
    ...
    IF p_scope = 'current' THEN
      -- APENAS deletar o parent, sem tocar nas filhas
      v_transaction_ids := ARRAY[p_transaction_id];
    ELSIF p_scope = 'current-and-remaining' THEN
      -- Deletar esta parent e as filhas PENDENTES com data >= data do parent
      SELECT ARRAY_AGG(id) INTO v_transaction_ids
      FROM transactions
      WHERE (id = p_transaction_id OR parent_transaction_id = p_transaction_id)
        ...
        AND user_id = p_user_id;
    ELSE -- 'all'
      -- Deletar parent + TODAS as filhas PENDENTES (preservando concluídas)
      SELECT ARRAY_AGG(id) INTO v_transaction_ids
      FROM transactions
      WHERE (id = p_transaction_id OR parent_transaction_id = p_transaction_id)
        AND status = 'pending'
