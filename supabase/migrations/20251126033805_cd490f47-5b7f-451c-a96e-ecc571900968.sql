-- Ajusta criação de transações fixas:
-- - Parent: sempre 'pending' (para edição na página Fixas)
-- - Primeira filha: usa status do formulário (aparece na página Transações)
-- - Demais filhas: sempre 'pending' (meses futuros)
...
BEGIN
  ...
  FOR i IN 1..(v_months_to_create - 1) LOOP
    ...
    -- PRIMEIRA FILHA: usa o status do formulário (p_status)
    -- DEMAIS FILHAS: sempre 'pending'
    INSERT INTO transactions (
      user_id, description, amount, date, type, category_id, account_id,
      ...
      p_account_id, 
      CASE WHEN v_is_first_child THEN p_status ELSE 'pending' END,
      true, v_parent_id
    );

    -- Recalcular saldo apenas se a primeira filha for 'completed'
    IF v_is_first_child AND p_status = 'completed' THEN
      PERFORM recalculate_account_balance(p_account_id);
    END IF;

    v_is_first_child := false;
    v_count := v_count + 1;
  END LOOP;
