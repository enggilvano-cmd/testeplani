...
-- - Parent: sempre 'pending', não aparece na página Transações (só Fixas)
-- - Primeira filha: mesma data da parent, usa status do formulário (aparece na página Transações)
-- - Demais filhas: meses subsequentes, sempre 'pending'
...
BEGIN
  ...
  -- A parent NÃO aparece na página Transações
  ...
  -- Esta primeira filha usa o status do formulário e aparece na página Transações
  INSERT INTO transactions (
    user_id, description, amount, date, type, category_id, account_id,
  ...
  FOR i IN 1..(v_months_to_create - 1) LOOP
    ...
    -- Criar transações filhas SEMPRE como 'pending' (meses futuros)
    INSERT INTO transactions (
      user_id, description, amount, date, type, category_id, account_id,
      status, is_fixed, parent_transaction_id
    ) VALUES (
